...
/// `GET /v1/subagent_profiles` returns the in-memory subagent profile
/// catalogue and is reachable via the assembled `configure_routes` app.
#[actix_web::test]
async fn subagent_profiles_route_returns_builtin_catalog_over_http() {
    let data_dir = tempdir().unwrap();
    bamboo_config::paths::init_bamboo_dir(data_dir.path().to_path_buf());
    let app_state = web::Data::new(AppState::new(data_dir.path().to_path_buf()).await.unwrap());
    let app = test::init_service(App::new().app_data(app_state).configure(configure_routes)).await;
    ...
    let profiles = body["profiles"]
        .as_array()
        .expect("response must include `profiles` array");
    assert!(
        profiles.len() >= 6,
        "expected at least 6 built-in profiles, got {}",
        profiles.len()
    );
    assert_eq!(body["fallback_id"], "general-purpose");
    assert_eq!(body["count"], profiles.len());

    // Required fields and explicit no-leak guarantee for system_prompt.
    for entry in profiles {
        ...
        assert!(
            ...
            "system_prompt must NOT be returned over HTTP"
        );
    }
}
