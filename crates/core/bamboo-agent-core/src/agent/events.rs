...
pub enum AgentEvent {
    ...
    /// Internal marker that binds subsequent visible [`Token`](Self::Token)
    /// events to one stable assistant message. Event forwarders MUST consume
    /// this variant locally: it is not part of the generic agent/account stream
    /// and must never be serialized to clients as a raw `AgentEvent`.
    #[doc(hidden)]
    VisibleMessageStart {
        /// Stable id reused by the final persisted assistant [`Message`].
        message_id: String,
        /// Timestamp reused by the final persisted assistant [`Message`].
        created_at: DateTime<Utc>,
    },

    /// Internal rollback for a visible message whose provider attempt is being
    /// retried. Like [`VisibleMessageStart`](Self::VisibleMessageStart), this is
    /// consumed by event forwarders and never crosses a public generic stream.
    #[doc(hidden)]
    VisibleMessageDiscard { message_id: String },
    ...
    /// Typed, bounded permission posture activated by an execution boundary.
    /// External workers send this once before vendor execution so the actor
    /// host can persist executor-specific mapping without relying on unknown
    /// fields attached to a generic progress event.
    PermissionPostureActivated {
        session_id: String,
        policy_revision: u64,
...
impl AgentEvent {
    ...
    pub fn session_id(&self) -> Option<&str> {
        match self {
            ...
            | AgentEvent::PermissionPostureActivated { session_id, .. }
...
mod tests {
    ...
    #[test]
    fn raw_child_and_streaming_payloads_are_never_durable_account_changes() {
        let raw_child = AgentEvent::SubAgentEvent {
            parent_session_id: "parent".to_string(),
            child_session_id: "child".to_string(),
            event: Box::new(AgentEvent::ReasoningToken {
                content: "private reasoning".to_string(),
            }),
        };
        for event in [
            AgentEvent::Token {
                content: "visible but ephemeral".to_string(),
            },
            ...
            assert!(
                !event.is_durable_change(),
                "unexpected durable event: {event:?}"
            );
        }

        assert!(AgentEvent::SubAgentStarted {
            parent_session_id: "parent".to_string(),
            child_session_id: "child".to_string(),
            title: None,
        }
        .is_durable_change());
        assert!(AgentEvent::SubAgentCompleted {
            parent_session_id: "parent".to_string(),
            child_session_id: "child".to_string(),
