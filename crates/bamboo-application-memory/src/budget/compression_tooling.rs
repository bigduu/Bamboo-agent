use bamboo_application_agent::{CompressionEvent, ConversationSummary, Message, Session};
use crate::budget::counter::{HeuristicTokenCounter, TokenCounter};
use crate::budget::limits::create_budget_for_model;
use crate::budget::{BudgetStrategy, TokenBudget};
use bamboo_application_agent::MessagePhase;
use chrono::Utc;
...
pub fn estimate_context_compression_exposure(
    ...
    let non_system_count = active_messages
        .iter()
        .filter(|m| !matches!(m.role, bamboo_application_agent::Role::System))
        .count();
...
fn build_compression_plan_with_summary_internal(
    ...
    let user_indexes = non_system
        ...
        .filter_map(|(index, message)| {
            matches!(message.role, bamboo_application_agent::Role::User).then_some(index)
        })
        .collect::<Vec<_>>();
    ...
    let anchor_index = if keep_user_count > 0 {
        ...
        non_system
            .iter()
            .rposition(|m| matches!(m.role, bamboo_application_agent::Role::User))
    ...
    let segmenter = crate::budget::segmenter::MessageSegmenter::new();
...
pub fn apply_compression_plan(session: &mut Session, plan: CompressionPlan) -> usize {
    ...
    let max_context_tokens = previous_usage
        ...
        .unwrap_or(0);
    session.token_usage = Some(bamboo_application_agent::TokenBudgetUsage {
        system_tokens,
...
pub fn summary_source_messages(session: &Session) -> Vec<Message> {
    session
        ...
        .iter()
        .filter(|message| !message.compressed)
        .filter(|message| !matches!(message.role, bamboo_application_agent::Role::System))
        .cloned()
        .collect()
}
...
pub fn build_summary_prompt(
    ...
    for message in messages {
        let role = match message.role {
            bamboo_application_agent::Role::System => continue,
            bamboo_application_agent::Role::User => "User",
            bamboo_application_agent::Role::Assistant => match message.phase {
                Some(MessagePhase::Commentary) => "Assistant Commentary",
                Some(MessagePhase::FinalAnswer) => "Assistant Final",
                None => "Assistant",
            },
            bamboo_application_agent::Role::Tool => "Tool Result",
        };
...
mod tests {
    use super::*;
    use bamboo_application_agent::TokenBudgetUsage;
