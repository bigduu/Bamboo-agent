...
pub fn get_default_config_path() -> PathBuf {
    bamboo_infrastructure_config::paths::bamboo_dir().join("model_limits.json")
}
...
mod tests {
    ...
    fn unified_config_loader_returns_none_when_absent() {
        ...
        let config = bamboo_infrastructure_config::Config::from_data_dir(Some(temp_dir.path().to_path_buf()));
    ...
    fn unified_config_loader_reads_valid_model_limits() {
        let temp_dir = tempfile::tempdir().expect("tempdir");
        let mut config = bamboo_infrastructure_config::Config::from_data_dir(Some(temp_dir.path().to_path_buf()));
        config.extra.insert(
