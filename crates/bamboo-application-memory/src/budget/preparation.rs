...
use bamboo_application_agent::{Role, Session};
use crate::budget::counter::TokenCounter;
use crate::budget::segmenter::{MessageSegment, MessageSegmenter};
...
///
/// ```ignore
/// use crate::budget::{TokenBudget, HeuristicTokenCounter, prepare_hybrid_context};
///
...
pub fn prepare_hybrid_context(
    ...
    let summary_message = session.conversation_summary.as_ref().map(|summary| {
        crate::budget::compression_tooling::compression_summary_message(
            &summary.content,
        )
    });
...
struct PromptCacheCompactionResult {
    messages: Vec<bamboo_application_agent::Message>,
...
fn maybe_compact_old_tool_outputs_for_prompt(
    session: &Session,
    mut active_messages: Vec<bamboo_application_agent::Message>,
    budget: &TokenBudget,
    counter: &dyn TokenCounter,
    summary_tokens: u32,
...
fn build_prompt_cache_candidates(
    messages: &[bamboo_application_agent::Message],
...
fn tool_call_name_index(messages: &[bamboo_application_agent::Message]) -> HashMap<String, String> {
...
mod tests {
    use super::*;
    use bamboo_application_agent::{ConversationSummary, Message, Role};
    use crate::budget::compression_summary_message;
    use crate::budget::counter::HeuristicTokenCounter;
    use crate::budget::counter::TokenCounter;
    use bamboo_application_agent::{FunctionCall, ToolCall};
    ...
    fn always_includes_system_messages() {
        ...
        assert!(prepared
            .messages
            .iter()
            .any(|m| m.role == bamboo_application_agent::Role::System));
    }
    ...
    fn preserves_recent_messages_when_truncating() {
        ...
        let last_user = prepared
            ...
            .iter()
            .rev()
            .find(|m| m.role == bamboo_application_agent::Role::User);
