...
use bamboo_application_agent::{Message, Role};
...
- Write in the same language as the original conversation"#;
    ...
    async fn collect_stream_response(
        ...
        let stream = self
            ...
            .await
            .map_err(|e| {
                crate::budget::types::BudgetError::TokenCountError(format!(
...
mod tests {
    use super::*;
    use bamboo_infrastructure_llm::{LLMChunk, LLMError, LLMRequestOptions, LLMStream};
    use bamboo_shared_types::ReasoningEffort;
    use async_trait::async_trait;
    use futures::stream;
    use std::sync::Mutex;
    ...
    impl LLMProvider for DummyProvider {
        async fn chat_stream(
            &self,
            _messages: &[Message],
            _tools: &[bamboo_application_agent::ToolSchema],
    ...
    impl LLMProvider for ReasoningCaptureProvider {
        ...
        async fn chat_stream_with_options(
            &self,
            messages: &[Message],
            tools: &[bamboo_application_agent::ToolSchema],
