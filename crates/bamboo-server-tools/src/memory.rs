...
use bamboo_application_tools::tools::session_memory::{
...
impl Tool for MemoryTool {
    ...
    fn call_mutability(&self, args: &serde_json::Value) -> bamboo_application_tools::ToolMutability {
        let action = args
            .get("action")
        ...
        match action.as_str() {
            "session_read" | "session_list_topics" | "query" | "get" | "inspect" => {
                bamboo_application_tools::ToolMutability::ReadOnly
            }
            _ => bamboo_application_tools::ToolMutability::Mutating,
        }
    }
    ...
    async fn execute_with_context(
        ...
        match parsed {
            ...
            MemoryArgs::Query {
                ...
                Ok(ToolResult {
                    ...
                    result: json!({
                        ...
                        "success": true,
                        "data": result,
                        "summary": bamboo_application_memory::memory_store::summary_json(result.returned_count, result.matched_count),
                        "warnings": [],
                    }).to_string(),
            ...
            MemoryArgs::Get {
                ...
                let (body, truncated) =
                    bamboo_application_memory::memory_store::truncate_chars(&doc.body, max_chars);
                doc.body = body;
                Ok(ToolResult {
                    success: true,
                    result: json!({
    ...
    async fn memory_session_append_enforces_shared_limit() {
        ...
        tool.execute_with_context(
            json!({
                ...
                "topic":"limit",
                "content":"x".repeat(bamboo_application_tools::tools::session_memory::MAX_SESSION_NOTE_CHARS - 1)
            }),
