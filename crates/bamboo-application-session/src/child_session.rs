...
pub fn replace_or_append_last_user_message(
    ...
    session.add_message(bamboo_application_agent::Message::user(content));
...
pub async fn create_child_action(
    ...
    use bamboo_application_runtime::runner::refresh_prompt_snapshot;
...
pub async fn get_child_action(
    ...
    Ok(json!({
        ...
        "latest_user_message": child
            .messages
            .iter()
            .rposition(|message| matches!(message.role, bamboo_application_agent::Role::User))
...
pub async fn send_message_to_child_action(
    ...
    child.add_message(bamboo_application_agent::Message::user(message.clone()));
    child
        .metadata
        .insert("last_run_status".to_string(), "pending".to_string());
    child.metadata.remove("last_run_error");
...
mod tests {
    ...
    fn truncate_after_last_user_removes_assistant_tail() {
        ...
        session.add_message(bamboo_application_agent::Message::system("system"));
        session.add_message(bamboo_application_agent::Message::user("task"));
        session.add_message(bamboo_application_agent::Message::assistant("done", None));
        ...
        assert!(matches!(
            ...
            bamboo_application_agent::Role::User
        ));
    }
    ...
    fn replace_or_append_last_user_message_replaces_existing() {
        let mut session = Session::new_child("child", "root", "test-model", "Child");
        session.add_message(bamboo_application_agent::Message::user("old"));
        session.add_message(bamboo_application_agent::Message::assistant("tail", None));
