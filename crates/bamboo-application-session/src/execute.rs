...
use crate::errors::ExecutePreparationError;
...
fn validate_image_fallback_for_session(
    session: &Session,
    image_fallback: Option<&bamboo_application_runtime::ImageFallbackConfig>,
) -> Result<(), String> {
    use bamboo_application_runtime::ImageFallbackMode;

    if matches!(
        image_fallback,
        Some(bamboo_application_runtime::ImageFallbackConfig {
        ...
        let images_seen = session
            ...
            .filter(|part| {
                matches!(
                    part,
                    bamboo_application_agent::MessagePart::ImageUrl { .. }
                )
            })
            .count();
