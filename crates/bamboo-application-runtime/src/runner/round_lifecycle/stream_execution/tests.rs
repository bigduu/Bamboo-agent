...
use bamboo_application_memory::budget::{PreparedContext, TokenUsageBreakdown};
use bamboo_application_agent::{AgentEvent, Message, Role, Session};
use bamboo_infrastructure_llm::{LLMChunk, LLMProvider, LLMRequestOptions, LLMStream};
    ...
    async fn chat_stream_with_options(
        ...
        *self
        ...
        let items = self
            ...
            .clone()
            .into_iter()
            .map(Ok::<LLMChunk, bamboo_infrastructure_llm::provider::LLMError>);
...
async fn execute_llm_stream_keeps_previous_response_id_when_local_summary_or_compression_is_active()
{
    ...
    session.conversation_summary =
        Some(bamboo_application_agent::agent::types::ConversationSummary::new(
