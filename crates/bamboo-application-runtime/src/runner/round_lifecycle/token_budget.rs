use bamboo_application_memory::budget::limits::{load_model_limits_from_unified_config, ModelLimit};
use bamboo_application_memory::budget::{ModelLimitsRegistry, TokenBudget};
use bamboo_application_agent::Session;
use bamboo_infrastructure_llm::provider::LLMProvider;
...
pub(super) async fn resolve_token_budget(
    ...
    let loaded_from_file = match registry.load_user_config().await {
        ...
        Err(error) => {
            tracing::warn!(
                ...
                bamboo_application_memory::budget::limits::get_default_config_path(),
                error
            );
            false
        }
    ...
    if !loaded_from_file {
        let unified_model_limits = match tokio::task::spawn_blocking(|| {
            let config = bamboo_infrastructure_config::Config::new();
    ...
    TokenBudget::with_safety_margin(
        model_limit.max_context_tokens,
        model_limit.get_max_output_tokens(),
        bamboo_application_memory::budget::BudgetStrategy::default(),
        model_limit.get_safety_margin(),
    )
}
...
mod tests {
    ...
    use bamboo_infrastructure_llm::provider::{LLMError, ProviderModelInfo, Result};
