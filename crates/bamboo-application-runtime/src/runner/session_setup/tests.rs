...
use bamboo_application_agent::tools::{FunctionSchema, ToolCall, ToolExecutor, ToolResult, ToolSchema};
...
impl ToolExecutor for StaticToolExecutor {
    async fn execute(
        &self,
        _call: &ToolCall,
    ) -> bamboo_application_agent::tools::executor::Result<ToolResult> {
        Ok(ToolResult {
            success: true,
            result: "ok".to_string(),
...
fn resolve_available_tool_schemas_uses_executor_when_registry_empty() {
    let config = crate::config::AgentLoopConfig::default();
...
fn resolve_available_tool_schemas_dedupes_and_merges_additional_entries() {
    let config = crate::config::AgentLoopConfig {
...
fn resolve_available_tool_schemas_includes_activated_discoverable_tools() {
    ...
    bamboo_application_tools::exposure::activate_discoverable_tools(
        &mut session,
        ["Sleep", "scheduler"],
    );
...
fn apply_system_prompt_contexts_persists_shared_prompt_snapshot() {
    let _lock = crate::tests::env_cache_lock_acquire();
    let config = bamboo_infrastructure_config::Config::default();
    config.publish_env_vars();

    let loop_config = crate::config::AgentLoopConfig {
...
fn refresh_prompt_snapshot_from_session_restores_prompt_memory_observability_from_metadata() {
    ...
    session.metadata.insert(
        ...
        serde_json::to_string(&bamboo_application_agent::PromptMemoryObservability {
...
fn apply_system_prompt_contexts_persists_runtime_prompt_metadata() {
    let _lock = crate::tests::env_cache_lock_acquire();
    let config_with_env = bamboo_infrastructure_config::Config {
    ...
    let mut session = Session::new("session-1", "model");
    let env_context = crate::context::build_env_prompt_context().unwrap_or_default();
    session.add_message(Message::system(format!(
        "Base prompt\n\n{}\nWorkspace path: {}\n{}\n{}\n\n{}",
        crate::context::WORKSPACE_CONTEXT_START_MARKER,
        workspace.display(),
        crate::context::WORKSPACE_CONTEXT_END_MARKER,
        crate::context::workspace_prompt_guidance(),
        env_context,
    )));
    let config = crate::config::AgentLoopConfig::default();
