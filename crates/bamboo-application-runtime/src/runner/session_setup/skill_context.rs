...
pub(super) async fn load_skill_context(
    ...
    if let Some(skill_manager) = config.skill_manager.as_ref() {
        let selected_skills: Vec<bamboo_application_skills::SkillDefinition> = skill_manager
        ...
        let context = bamboo_application_skills::context::build_skill_context(&selected_skills);
        if !context.is_empty() {
            tracing::info!(
