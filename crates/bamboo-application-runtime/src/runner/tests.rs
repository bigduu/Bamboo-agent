...
use bamboo_application_agent::tools::{FunctionCall, Tool, ToolError, ToolExecutionContext, ToolResult};
use bamboo_application_agent::{Message, Session};
use bamboo_infrastructure_llm::{LLMChunk, LLMProvider, LLMStream};
use bamboo_application_tools::BuiltinToolExecutorBuilder;
...
async fn agent_loop_passes_session_id_into_tool_execution_context() {
    struct QueueProvider {
        ...
        queue: Mutex<Vec<Vec<bamboo_infrastructure_llm::provider::Result<LLMChunk>>>>,
    }
    ...
    let tool_call = bamboo_application_agent::tools::ToolCall {
