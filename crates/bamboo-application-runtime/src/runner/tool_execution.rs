...
use bamboo_application_metrics::{MetricsCollector, RoundStatus as MetricsRoundStatus};
...
fn scheduling_mode_for_tool_call(
    ...
    let normalized = bamboo_application_tools::normalize_tool_ref(&tool_call.function.name)
        .unwrap_or_else(|| tool_call.function.name.trim().to_string());

    let canonical = bamboo_application_tools::resolve_alias(&normalized)
    ...
    if bamboo_application_tools::parallel::ToolCallRuntime::supports_parallel(tools, &effective_call) {
...
mod tests {
    ...
    use bamboo_application_agent::tools::{FunctionCall, ToolCall, ToolExecutor};
