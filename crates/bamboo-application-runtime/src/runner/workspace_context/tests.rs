...
fn should_not_apply_workspace_update_when_target_is_inside_workspace() {
    ...
    let workspace_display = bamboo_infrastructure_config::paths::path_to_display_string(&workspace);
    let file_display = bamboo_infrastructure_config::paths::path_to_display_string(&file_path);
...
fn should_apply_workspace_update_when_target_is_outside_workspace() {
    ...
    let workspace_display = bamboo_infrastructure_config::paths::path_to_display_string(&workspace);
    let notebook_display = bamboo_infrastructure_config::paths::path_to_display_string(&notebook_path);
...
fn upsert_workspace_context_replaces_existing_segment() {
    ...
    assert!(updated.contains(crate::context::WORKSPACE_CONTEXT_START_MARKER));
    assert!(updated.contains(crate::context::WORKSPACE_CONTEXT_END_MARKER));
...
fn apply_workspace_path_to_session_updates_metadata_and_prompt() {
    ...
    assert!(system_content.contains(crate::context::WORKSPACE_CONTEXT_START_MARKER));
    assert!(system_content.contains(crate::context::WORKSPACE_CONTEXT_END_MARKER));
