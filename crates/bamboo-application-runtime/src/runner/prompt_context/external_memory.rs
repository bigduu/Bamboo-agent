...
use bamboo_application_agent::{PromptMemoryObservability, Session};
...
pub(super) async fn inject_external_memory_into_system_message_with_store(
    ...
    let Some(system_message_index) = session
        .messages
        .iter()
        .position(|message| matches!(message.role, bamboo_application_agent::Role::System))
    else {
        return;
    };
...
fn latest_user_query_text(session: &Session) -> Option<String> {
    session
        ...
        .iter()
        .rev()
        .find(|message| matches!(message.role, bamboo_application_agent::Role::User))
