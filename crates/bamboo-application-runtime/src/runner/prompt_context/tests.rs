...
async fn inject_external_memory_includes_global_dream_fallback_and_session_note_when_project_unknown(
    ...
    let store = bamboo_application_memory::memory_store::MemoryStore::new(temp_dir.path());
    store
    ...
    let mut session = bamboo_application_agent::Session::new("session-dream-test", "test-model");
    session.add_message(bamboo_application_agent::Message::system("Base prompt"));

    super::inject_external_memory_into_system_message_with_store(
        &mut session,
        &store,
        crate::config::PromptMemoryFlags::default(),
        None,
    )
    .await;
...
async fn inject_external_memory_includes_project_memory_index_and_omits_global_dream_fallback() {
    ...
    store
        .write_memory(
            bamboo_application_memory::memory_store::MemoryScope::Project,
    ...
    let mut session = bamboo_application_agent::Session::new("session-project-memory", "test-model");
    session.add_message(bamboo_application_agent::Message::system("Base prompt"));
...
async fn inject_external_memory_excludes_other_project_memory_index_content() {
    ...
    let project_key_b = bamboo_application_memory::memory_store::project_key_from_path(&workspace_b);
    ...
    let mut session = bamboo_application_agent::Session::new("session-project-a", "test-model");
    session.add_message(bamboo_application_agent::Message::system("Base prompt"));
    session.metadata.insert(
        "workspace_path".to_string(),
...
async fn inject_external_memory_truncates_project_memory_index_and_adds_freshness_note() {
    ...
    std::fs::write(
        views_dir.join(bamboo_application_memory::memory_store::MEMORY_VIEW_FILE),
        large_view,
    )
    .expect("write memory view");

    let mut session =
        bamboo_application_agent::Session::new("session-project-memory-truncated", "test-model");
    session.add_message(bamboo_application_agent::Message::system("Base prompt"));
    session.metadata.insert(
        "workspace_path".to_string(),
        workspace.to_string_lossy().to_string(),
...
async fn inject_external_memory_truncates_multi_topic_content_and_is_idempotent() {
    ...
    let mut session = bamboo_application_agent::Session::new("session-memory-many", "test-model");
    session.add_message(bamboo_application_agent::Message::system("Base prompt"));
...
async fn inject_external_memory_renders_relevant_memory_section_for_project_hits() {
    ...
    store
        .write_memory(
            bamboo_application_memory::memory_store::MemoryScope::Project,
            Some(project_key.as_str()),
            bamboo_application_memory::memory_store::DurableMemoryType::Feedback,
    ...
    let mut session = bamboo_application_agent::Session::new("session-recall-project", "test-model");
    session.add_message(bamboo_application_agent::Message::system("Base prompt"));
    session.metadata.insert(
        "workspace_path".to_string(),
        workspace.to_string_lossy().to_string(),
    );
    session.add_message(bamboo_application_agent::Message::user(
...
async fn inject_external_memory_adds_stale_guidance_for_old_relevant_memory_hits() {
    ...
    let mut session = bamboo_application_agent::Session::new("session-recall-stale", "test-model");
    session.add_message(bamboo_application_agent::Message::system("Base prompt"));
    session.metadata.insert(
        "workspace_path".to_string(),
        workspace.to_string_lossy().to_string(),
    );
    session.add_message(bamboo_application_agent::Message::user("release freeze policy"));
...
async fn inject_external_memory_omits_relevant_memory_section_when_no_match_exists() {
    ...
    let mut session = bamboo_application_agent::Session::new("session-recall-none", "test-model");
    session.add_message(bamboo_application_agent::Message::system("Base prompt"));
    session.metadata.insert(
        "workspace_path".to_string(),
        workspace.to_string_lossy().to_string(),
...
async fn inject_external_memory_limits_relevant_memories_to_top_k() {
    ...
    let mut session = bamboo_application_agent::Session::new("session-recall-topk", "test-model");
    session.add_message(bamboo_application_agent::Message::system("Base prompt"));
    session.metadata.insert(
        "workspace_path".to_string(),
        workspace.to_string_lossy().to_string(),
    );
    session.add_message(bamboo_application_agent::Message::user("release freeze"));
...
async fn inject_external_memory_uses_global_relevant_memory_fallback_only_when_project_has_no_hits()
{
    ...
    store
        .write_memory(
            bamboo_application_memory::memory_store::MemoryScope::Global,
            None,
            bamboo_application_memory::memory_store::DurableMemoryType::Reference,
    ...
    let mut session = bamboo_application_agent::Session::new("session-recall-fallback", "test-model");
    session.add_message(bamboo_application_agent::Message::system("Base prompt"));
    session.metadata.insert(
        "workspace_path".to_string(),
        workspace.to_string_lossy().to_string(),
    );
    session.add_message(bamboo_application_agent::Message::user("release checklist"));
...
async fn inject_external_memory_prefers_project_dream_over_global_fallback() {
    ...
    let mut session = bamboo_application_agent::Session::new("session-project-dream", "test-model");
    session.add_message(bamboo_application_agent::Message::system("Base prompt"));
    session.metadata.insert(
        "workspace_path".to_string(),
        workspace.to_string_lossy().to_string(),
...
async fn inject_external_memory_uses_global_dream_fallback_when_project_dream_and_index_are_missing(
    ...
    let mut session =
        bamboo_application_agent::Session::new("session-global-dream-fallback", "test-model");
    session.add_message(bamboo_application_agent::Message::system("Base prompt"));
    session.metadata.insert(
        "workspace_path".to_string(),
        workspace.to_string_lossy().to_string(),
...
async fn inject_external_memory_omits_project_index_when_project_prompt_injection_disabled() {
    ...
    let mut session = bamboo_application_agent::Session::new("session-no-project-index", "test-model");
    session.add_message(bamboo_application_agent::Message::system("Base prompt"));
    session.metadata.insert(
        "workspace_path".to_string(),
        workspace.to_string_lossy().to_string(),
    ...
    super::inject_external_memory_into_system_message_with_store(
        &mut session,
        &store,
        crate::config::PromptMemoryFlags {
    ...
    let observability = session
        ...
        .and_then(|raw| {
            serde_json::from_str::<bamboo_application_agent::PromptMemoryObservability>(raw).ok()
        })
...
async fn inject_external_memory_omits_relevant_recall_and_uses_global_dream_when_project_first_disabled(
    ...
    let mut session = bamboo_application_agent::Session::new("session-global-dream-mode", "test-model");
    session.add_message(bamboo_application_agent::Message::system("Base prompt"));
    session.metadata.insert(
        "workspace_path".to_string(),
        workspace.to_string_lossy().to_string(),
    );
    session.add_message(bamboo_application_agent::Message::user("concise answers"));
...
async fn inject_external_memory_uses_model_rerank_for_relevant_memories_when_enabled() {
    ...
    let mut session = bamboo_application_agent::Session::new("session-rerank-recall", "test-model");
    session.add_message(bamboo_application_agent::Message::system("Base prompt"));
    session.metadata.insert(
        "workspace_path".to_string(),
        workspace.to_string_lossy().to_string(),
