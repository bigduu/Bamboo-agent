...
fn should_retry_round_error(error: &bamboo_application_agent::AgentError) -> bool {
    use bamboo_application_agent::AgentError;
...
fn is_overflow_recoverable(error: &bamboo_application_agent::AgentError) -> bool {
    matches!(error, bamboo_application_agent::AgentError::LLMOverflow(_))
}
...
pub(super) async fn run_rounds(
    ...
    for round in 0..config.max_rounds {
        ...
        let mut terminal_error: Option<bamboo_application_agent::AgentError> = None;
                ...
                Err(error) => {
                    if is_overflow_recoverable(&error) && !overflow_recovery_attempted_in_round {
                        ...
                        if !state.overflow_recovery.can_attempt_recovery() {
                            let breaker_error = bamboo_application_agent::AgentError::LLMOverflow(
                                format!(
        ...
        let Some(round_flow_outcome) = round_flow_outcome else {
            let error =
                bamboo_application_agent::AgentError::LLM("round completed without outcome".to_string());
            record_round_failure(
                state.metrics_collector.as_ref(),
                &round_id,
                &state.session_id,
...
mod tests {
    ...
    use bamboo_application_agent::AgentError;
    use crate::runner::loop_execution::startup::OverflowRecoveryState;
