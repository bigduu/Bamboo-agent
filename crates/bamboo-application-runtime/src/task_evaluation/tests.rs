...
use bamboo_application_agent::{AgentEvent, Message};
use bamboo_domain_session::{TaskItemStatus, TaskItem, TaskList};
use bamboo_infrastructure_llm::{LLMChunk, LLMError, LLMProvider, LLMRequestOptions, LLMStream};
use crate::task_context::{TaskLoopContext, TaskLoopItem, ToolCallRecord};
...
fn create_test_context() -> TaskLoopContext {
    let mut session = bamboo_application_agent::Session::new("test", "test-model");
    ...
    context.items = vec![TaskLoopItem {
        ...
        active_form: None,
        parent_id: None,
        phase: bamboo_domain_session::task::TaskPhase::Execution,
        priority: bamboo_domain_session::task::TaskPriority::Medium,
...
fn build_evaluation_messages_contains_context_and_rules() {
    ...
    let session = bamboo_application_agent::Session::new("test", "test-model");
...
async fn task_evaluation_uses_explicit_model_parameter_for_provider_request() {
    let context = create_test_context();
    let session = bamboo_application_agent::Session::new("test-session", "session-model");
