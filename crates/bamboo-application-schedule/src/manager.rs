...
use bamboo_application_runtime::execution::{
    create_event_forwarder, finalize_runner, get_or_create_event_sender, try_reserve_runner,
    AgentRunner,
};
use bamboo_application_runtime::ExecuteRequest;
use bamboo_shared_types::reasoning::ReasoningEffort;

use bamboo_domain_schedule::{ScheduleRunConfig, ScheduleRunStatus};
use crate::store::{ClaimedScheduleRun, ScheduleStore};
use crate::trigger_engine::DynTriggerEngine;
...
async fn run_schedule_job(
    ...
    let mut session = crate::session_factory::create_schedule_session(
        &job,
        &resolved.model,
