...
pub fn expandable_tool_short_description(name: &str) -> Option<&'static str> {
    match canonical_tool_name(name).as_str() {
        ...
        "session_history" => Some("Search the current Bamboo Session's stored messages, including compressed history. Root schemas may also offer cross-session list/inspect/search/export actions. Distinct from `memory` (durable knowledge)."),
