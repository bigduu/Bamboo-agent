...
impl A2AExternalChildRunner {
    ...
    pub fn build_client_config(
    ...
    ) -> bamboo_a2a::A2AClientResult<A2AClientConfig> {
        ...
        let agent_card_url = profile.agent_card_url.clone().ok_or_else(|| {
            bamboo_a2a::A2AClientError::InvalidAgentCard(format!(
...
async fn handle_non_streaming(
    ...
    let synthetic = bamboo_a2a::types::StreamResponse {
    ...
    if let Some(task) = &response.task {
        let status_update = bamboo_a2a::types::StreamResponse {
            task: None,
            message: None,
            status_update: Some(bamboo_a2a::types::TaskStatusUpdateEvent {
...
async fn handle_streaming(
    client: &A2AJsonRpcClient,
    mut stream: bamboo_a2a::A2AStream,
    event_tx: mpsc::Sender<AgentEvent>,
    cancel_token: CancellationToken,
    session: &mut bamboo_agent_core::Session,
    tenant: Option<String>,
...
async fn recover_task_state(
    ...
    Ok(bamboo_a2a::types::StreamResponse {
        task: Some(task),
        message: None,
        status_update: None,
        artifact_update: None,
