...
//!
//! ```rust,ignore
//! use bamboo_infrastructure_process::registry::{ProcessRegistry, ProcessRegistrationConfig};
...
impl ProcessRegistry {
    ...
    pub async fn kill_process_by_pid(&self, run_id: i64, pid: u32) -> Result<bool, String> {
        ...
        let kill_result = if cfg!(target_os = "windows") {
            ...
            crate::process_utils::trace_windows_command(
                "process_registry.kill_process_by_pid",
                "taskkill",
                ["/F", "/PID", pid_str.as_str()],
            );
            let mut command = std::process::Command::new("taskkill");
            crate::process_utils::hide_window_for_std_command(&mut command);
