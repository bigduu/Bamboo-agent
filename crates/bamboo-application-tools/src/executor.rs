...
use bamboo_application_tool_types::tool_names::{normalize_builtin_alias, resolve_alias};
...
impl ToolExecutor for BuiltinToolExecutor {
    ...
    fn call_mutability(&self, call: &ToolCall) -> crate::ToolMutability {
        ...
        let args =
            bamboo_application_agent::parse_tool_args_best_effort(&call.function.arguments).0;
        self.registry
            .get(&canonical)
...
mod tests {
    use super::*;
    use bamboo_application_agent::FunctionCall;
    use bamboo_application_agent::ToolExecutionContext;
    use bamboo_application_agent::AgentEvent;
    use bamboo_application_tool_types::tool_names::{normalize_tool_ref, BUILTIN_TOOL_NAMES};
    use serde_json::json;
    use std::sync::Arc;
    use tokio::fs;
    use tokio::sync::mpsc;
