    ...
    match tool_name {
        ...
        "session_note" | "memory_note" => {
            ...
            if matches!(action.as_str(), "append" | "replace" | "clear") {
                let notes_dir = bamboo_infrastructure_config::paths::bamboo_dir()
                    .join("memory")
                    .join("v1")
                    .join("sessions");
                let notes_path = bamboo_infrastructure_config::paths::path_to_display_string(&notes_dir);
                Ok(Some(vec![PermissionContext::new(
                    PermissionType::WriteFile,
                    notes_path.clone(),
        ...
        "memory" => {
            let action = required_string_arg(args, "action")?
                .trim()
                .to_ascii_lowercase();
            let bamboo_dir = bamboo_infrastructure_config::paths::bamboo_dir();
            let session_memory_dir = bamboo_infrastructure_config::paths::path_to_display_string(
                &bamboo_dir.join("memory").join("v1").join("sessions"),
            );
            let durable_memory_dir = bamboo_infrastructure_config::paths::path_to_display_string(
