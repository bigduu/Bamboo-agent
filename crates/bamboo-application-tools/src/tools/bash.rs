...
use bamboo_infrastructure_process::{
...
impl BashTool {
    ...
    fn python_diagnostics_json(
        diagnostics: &bamboo_infrastructure_process::PythonDiscoveryDiagnostics,
    ...
    fn environment_json(
        diagnostics: &bamboo_infrastructure_process::CommandEnvironmentDiagnostics,
    ...
    fn resolve_cwd(session_workspace: &Path, workdir: Option<&str>) -> Result<PathBuf, ToolError> {
        ...
        let metadata = std::fs::metadata(&resolved).map_err(|error| {
            ToolError::InvalidArguments(format!(
                "Invalid workdir '{}': {}",
                bamboo_infrastructure_config::paths::path_to_display_string(&resolved),
                error
            ))
        })?;
        if !metadata.is_dir() {
    ...
    async fn prepare_environment() -> PreparedCommandEnvironment {
        let overrides = bamboo_infrastructure_config::Config::current_env_vars();
    ...
    async fn run_foreground(
        ...
        let cwd_display = bamboo_infrastructure_config::paths::path_to_display_string(cwd);
        ...
        if parsed.run_in_background.unwrap_or(false) {
            ...
            return Ok(ToolResult {
                ...
                result: json!({
                    ...
                    "status": "running",
                    "cwd": bamboo_infrastructure_config::paths::path_to_display_string(&cwd),
...
mod tests {
    ...
    fn invalid_utf8_stderr_command() -> String {
        let shell = bamboo_infrastructure_process::preferred_bash_shell();
        if shell.arg == "-lc" {
            "printf '\\377\\n' 1>&2".to_string()
        } else {
    ...
    async fn bash_resolves_relative_workdir_from_session_workspace() {
        ...
        let payload: Value = serde_json::from_str(&result.result).unwrap();
        let expected = bamboo_infrastructure_config::paths::path_to_display_string(&nested.canonicalize().unwrap());
