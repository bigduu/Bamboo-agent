...
impl GrepTool {
    ...
    fn format_content_hits(
        ...
        selected_lines
            .into_iter()
            .map(|idx| {
                let display_path = bamboo_infrastructure_config::paths::path_to_display_string(path);
                if line_numbers {
...
impl Tool for GrepTool {
    ...
    async fn execute_with_context(
        ...
        for file in files {
            ...
            matched_files.push(bamboo_infrastructure_config::paths::path_to_display_string(&file));
            count_rows.push(format!(
                "{}:{}",
                bamboo_infrastructure_config::paths::path_to_display_string(&file),
                match_count
            ));
