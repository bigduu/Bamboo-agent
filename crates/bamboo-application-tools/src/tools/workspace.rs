            ...
            Some(path) => {
                ...
                Ok(ToolResult {
                    ...
                    result: json!({
                        ...
                        "workspace": bamboo_infrastructure_config::paths::path_to_display_string(&absolute_path)
                    })
                    .to_string(),
            ...
            None => {
                if let Some(session_id) = ctx.session_id {
                    if let Some(workspace) = workspace_state::get_workspace(session_id) {
                        return Ok(ToolResult {
                            success: true,
                            result: bamboo_infrastructure_config::paths::path_to_display_string(&workspace),
                ...
                match std::env::current_dir() {
                    Ok(dir) => Ok(ToolResult {
                        success: true,
                        result: bamboo_infrastructure_config::paths::path_to_display_string(&dir),
                        display_preference: None,
                    }),
...
mod tests {
    ...
    async fn workspace_set_changes_session_workspace() {
        ...
        assert_eq!(
            get_result.result,
            bamboo_infrastructure_config::paths::path_to_display_string(&expected)
        );
    }
