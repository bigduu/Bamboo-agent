    ...
    async fn execute_with_context(
            ...
            .filter_map(|entry| entry.ok())
        {
            ...
            let modified = entry
                ...
                .unwrap_or(0);
            matches.push((
                bamboo_infrastructure_config::paths::path_to_display_string(Path::new(path)),
                modified,
            ));
        }
...
mod tests {
    use super::GlobTool;
    use bamboo_application_agent::Tool;
    use serde_json::json;

    fn result_lines(result: &bamboo_application_agent::ToolResult) -> Vec<&str> {
        result
            .result
            .lines()
