...
pub fn load_builtin_skill_bundles() -> SkillResult<Vec<BuiltinSkillBundle>> {
    ...
    let skills_dir = bamboo_infrastructure_config::paths::bamboo_dir().join("skills");
    let skills_dir_display = bamboo_infrastructure_config::paths::path_to_display_string(&skills_dir);
