...
//! internal `bamboo_domain_session::Message` fields (like `id` / `created_at`).

use bamboo_domain_session::{Role, Message};
...
pub fn parse_openai_compat_chunk(chunk: OpenAICompatStreamChunk) -> LLMChunk {
    ...
    if let Some(tool_calls) = &choice.delta.tool_calls {
        let calls: Vec<bamboo_domain_session::ToolCall> = tool_calls
            .iter()
            .map(|tc| bamboo_domain_session::ToolCall {
                id: tc.id.clone().unwrap_or_default(),
                tool_type: tc
                    .tool_type
                    .clone()
                    .unwrap_or_else(|| "function".to_string()),
                function: bamboo_domain_session::FunctionCall {
                    name: tc
                        .function
                        .as_ref()
...
mod tests {
    ...
    use bamboo_application_tool_types::{FunctionSchema, ToolSchema};    use bamboo_domain_session::Message;
    use crate::types::LLMChunk;
