...
use bamboo_infrastructure_config::{
...
pub fn apply_overrides_to_header_map(
    ...
    let env_vars = bamboo_infrastructure_config::Config::current_env_vars();
    ...
    match expr {
        ...
        TemplateExpr::Structured(spec) => match spec {
            ...
            TemplateExprSpec::Generated { generator } => Some(match generator {
                bamboo_infrastructure_config::GeneratedValue::Uuid => Uuid::new_v4().to_string(),
                bamboo_infrastructure_config::GeneratedValue::UnixMs => Utc::now().timestamp_millis().to_string(),
            }),
