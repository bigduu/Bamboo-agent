...
use bamboo_domain_session::{Role, Message, MessagePhase};
...
impl ResponsesSseParser {
    ...
    fn finalize_tool_call(&mut self, item_id: &str) -> Option<bamboo_domain_session::ToolCall> {
        let acc = self.fn_calls.remove(item_id)?;
        Some(bamboo_domain_session::ToolCall {
            id: acc.call_id?,
...
mod tests {
    ...
    use bamboo_application_tool_types::{FunctionSchema, ToolSchema};    use bamboo_domain_session::MessagePhase;
