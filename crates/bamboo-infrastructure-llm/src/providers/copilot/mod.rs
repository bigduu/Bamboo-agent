...
impl CopilotProvider {
    ...
    fn infer_request_initiator(messages: &[Message]) -> &'static str {
        messages
            .iter()
            .rev()
            .find(|message| !matches!(message.role, bamboo_domain_session::Role::System))
            .map(|message| match message.role {
                bamboo_domain_session::Role::User => "user",
                bamboo_domain_session::Role::Assistant
                | bamboo_domain_session::Role::Tool => "agent",
                bamboo_domain_session::Role::System => "user",
            })
            .unwrap_or("user")
    }
    ...
    fn infer_openai_intent(messages: &[Message], tools: &[ToolSchema]) -> &'static str {
        let has_agent_activity = messages.iter().any(|message| {
            matches!(
                message.role,
                bamboo_domain_session::Role::Assistant | bamboo_domain_session::Role::Tool
            ) || message
                .tool_calls
                .as_ref()
