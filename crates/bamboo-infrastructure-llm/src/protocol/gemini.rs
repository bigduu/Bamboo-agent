...
use bamboo_domain_session::{FunctionCall, ToolCall};
use bamboo_application_tool_types::{FunctionSchema, ToolSchema};
use bamboo_domain_session::{Message, Role};
    ...
    fn from_provider(content: GeminiContent) -> ProtocolResult<Self> {
        ...
        for part in content.parts {
            if let Some(text) = part.text {
                ...
                content_parts.push(bamboo_domain_session::MessagePart::Text { text });
            }

            if let Some(inline_data) = part.inline_data {
                if let Some(url) = inline_data_to_data_url(&inline_data) {
                    has_image_parts = true;
                    content_parts.push(bamboo_domain_session::MessagePart::ImageUrl {
                        image_url: bamboo_domain_session::ImageUrlRef { url, detail: None },
                    });
                }
            }
            ...
            if let Some(file_data) = part.file_data {
                ...
                if !file_uri.is_empty() {
                    has_image_parts = true;
                    content_parts.push(bamboo_domain_session::MessagePart::ImageUrl {
                        image_url: bamboo_domain_session::ImageUrlRef {
...
fn message_content_part_to_gemini_part(
    part: &bamboo_domain_session::MessagePart,
) -> Option<GeminiPart> {
    match part {
        bamboo_domain_session::MessagePart::Text { text } => Some(GeminiPart {
        ...
        bamboo_domain_session::MessagePart::ImageUrl { image_url } => {
