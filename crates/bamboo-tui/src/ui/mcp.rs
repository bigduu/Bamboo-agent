...
pub fn render(f: &mut Frame, area: Rect, app: &App) {
    if app.mcp.loading && app.mcp.servers.is_empty() {
        let loading = Paragraph::new("Loading MCP servers...")
    ...
    let header = Paragraph::new(Line::from(vec![
        Span::styled(" MCP Servers", Style::default().fg(colors::BRAND).add_modifier(Modifier::BOLD)),
        Span::raw("   "),
        Span::styled("[r] Refresh", Style::default().fg(colors::INACTIVE)),
        Span::raw("  "),
        Span::styled("[t] Tools", Style::default().fg(colors::INACTIVE)),
        Span::raw("  "),
        Span::styled("[Enter] Connect/Disc", Style::default().fg(colors::INACTIVE)),
    ]));
