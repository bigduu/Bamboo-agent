...
pub fn render(f: &mut Frame, area: Rect, app: &App) {
    if app.config.loading && app.config.config.is_none() {
        let loading = Paragraph::new("Loading config...")
    ...
    let config_text = match &app.config.config {
        Some(val) => serde_json::to_string_pretty(val).unwrap_or_else(|_| "Invalid JSON".to_string()),
    ...
    let lines: Vec<Line> = config_text
        ...
        .map(|line| {
            ...
            if trimmed.starts_with('"') && trimmed.contains(':') {
                Line::from(Span::styled(line.to_string(), Style::default().fg(colors::BRAND)))
            } else if trimmed.starts_with('"') && trimmed.ends_with('"') {
                Line::from(Span::styled(line.to_string(), Style::default().fg(colors::SUCCESS)))
            } else if trimmed == "true" || trimmed == "false" {
                Line::from(Span::styled(line.to_string(), Style::default().fg(colors::WARNING)))
            } else if trimmed.parse::<f64>().is_ok() {
                Line::from(Span::styled(line.to_string(), Style::default().fg(colors::USER_PREFIX)))
            } else {
