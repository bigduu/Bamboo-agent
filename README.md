...
Jiandu defaults to the independent `~/.jiandu` data root. Bamboo configuration, sessions, and the prospective-record Ledger remain under `~/.bamboo`; the two stores are not mixed. For an isolated managed-host or acceptance run, `BAMBOO_JIANDU_DATA_DIR` may select a non-empty absolute Jiandu root for that server process. Invalid values stop startup before memory initialization. This is an isolation boundary, not a second persistence mode or a migration mechanism, and `--data-dir` continues to control Bamboo data only.
