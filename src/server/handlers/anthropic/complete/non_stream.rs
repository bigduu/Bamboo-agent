...
use crate::server::handlers::anthropic::{conversion::convert_complete_response, usage};
...
pub(super) async fn handle_non_streaming_complete(
    ...
    let response = match convert_complete_response(completion, &response_model) {
        ...
        Err(error) => {
            ...
            return Ok(crate::server::handlers::anthropic::errors::anthropic_error_response(error));
        }
    };
...
fn convert_tool_calls(
    calls: Vec<crate::agent::core::tools::ToolCall>,
) -> Vec<crate::agent::llm::api::models::ToolCall> {
    calls
        .into_iter()
        .map(|tool_call| crate::agent::llm::api::models::ToolCall {
            id: tool_call.id,
