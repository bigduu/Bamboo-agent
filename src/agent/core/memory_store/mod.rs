...
pub fn project_key_from_path(path: &Path) -> String {
    ...
    if let Some(root) = find_git_root(&canonical) {
        if let Some(name) = root.file_name().and_then(|value| value.to_str()) {
            ...
            if let Some(hash) =
                short_stable_hash(&crate::core::paths::path_to_display_string(&root))
            {
                key.push('-');
                key.push_str(&hash);
            }
    ...
    if let Some(name) = canonical.file_name().and_then(|value| value.to_str()) {
        let mut key = sanitize_component(name);
        if let Some(hash) =
            short_stable_hash(&crate::core::paths::path_to_display_string(&canonical))
        {
            key.push('-');
            key.push_str(&hash);
        }
    ...
    let raw = crate::core::paths::path_to_display_string(&canonical);
    format!(
        "path-{}",
