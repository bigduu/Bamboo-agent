...
async fn test_get_workspace_suggestions_endpoint() {
    ...
    let has_home = suggestions
        .iter()
        .any(|suggestion| suggestion["suggestion_type"] == "home");
...
async fn test_get_workspace_suggestions_includes_recent() {
    ...
    let suggestions = result["suggestions"]
        .as_array()
        .expect("suggestions should be array");
    let has_recent = suggestions.iter().any(|suggestion| {
        suggestion["path"] == temp_path && suggestion["suggestion_type"] == "recent"
    });
