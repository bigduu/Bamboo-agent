...
async fn test_set_bamboo_config_allows_incomplete_provider_config() {
    ...
    // Setup flows often set provider before credentials are provided.
    let set_req = test::TestRequest::post()
        .uri("/v1/bamboo/config")
        .set_json(&json!({
            "provider": "anthropic",
...
async fn test_set_proxy_auth_does_not_fail_when_provider_unconfigured() {
    ...
    // Intentionally set an incomplete provider config (no API key).
    let set_req = test::TestRequest::post()
        .uri("/v1/bamboo/config")
        .set_json(&json!({
            "provider": "anthropic"
    ...
    // Proxy auth should be storable even if provider can't initialize yet.
